...
void ModelRenderer::RenderModel(Model const& model)
{
	...
	auto const* pVertexPointer = static_cast<GLubyte const*>
		(model.GetVertexBuffer().GetBufferPointer());
	auto const* pIndexPointer = static_cast<GLubyte const*>
	...
	for (size_t i = 0; i < meshCount; ++i)
	{
		...
		const int stride = meshUsesTexture ? sizeof(TexturedVertex) : sizeof(Vertex);
		...
		{
			glLockArraysEXT(0, static_cast<int>(mesh.GetVertexCount()));
