...
public:
    ...
    [[nodiscard]] Vector4f Shade(ShadeContext const &shadeContext) const override {
        ...
        for (size_t i = 0; i < numLights; ++i) {
            ...
            const Vector4f diffuse = static_cast<float>(nDotL * lightIntensity) * light.GetDiffuseIntensity() *
                                     m_material.GetDiffuseColor();
            shadedColor += diffuse;
            ...
            const Vector4f specular =
                    static_cast<float>(std::pow(nDotH, shiness) * lightIntensity) * light.GetSpecularIntensity() *
                    m_material.GetSpecularColor();
