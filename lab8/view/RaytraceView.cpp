...
void RaytraceView::AddSomeMetaball() {
    ...
    const Metasphere metasphere2{{-0.5, 1.0, 0}, 1.0, 1.5};
    const Metasphere metasphere3{{-3, 1.0, 0}, 1.0, 1.0};
    AddMetaball(shader, {metasphere1, metasphere2, metasphere3}, {1, 1, 1}, transform);
}

SceneObject &
RaytraceView::AddPlane(const std::shared_ptr<IShader const> &shader, double a, double b, double c, double d,
                       Matrix4d const &transform) {
    return AddSceneObject(std::make_shared<Plane>(a, b, c, d, transform), shader);
}

SceneObject &
RaytraceView::AddSceneObject(std::shared_ptr<IGeometryObject const> object, std::shared_ptr<IShader const> shader) {
...
SceneObject &
RaytraceView::AddParaboloid(std::shared_ptr<IShader const> const &shader, double scale, Vector3d const &center,
                            Matrix4d const &transform) {
    return AddSceneObject(std::make_shared<Paraboloid>(scale, center, transform), shader);
}

SceneObject &
RaytraceView::AddTorus(std::shared_ptr<IShader const> const &shader, double majorRadius, double minorRadius,
                       Vector3d const &center, Matrix4d const &transform) {
    return AddSceneObject(std::make_shared<Torus>(majorRadius, minorRadius, center, transform), shader);
}

SceneObject &
RaytraceView::AddMetaball(std::shared_ptr<IShader const> const &shader, std::vector<Metasphere> const &spheres,
                          Vector3d const &center, Matrix4d const &transform) {
    return AddSceneObject(std::make_shared<Metaball>(spheres, center, transform), shader);
}

SceneObject &
RaytraceView::AddSphere(std::shared_ptr<IShader const> const &shader, double radius, Vector3d const &center,
                        Matrix4d const &transform) {
